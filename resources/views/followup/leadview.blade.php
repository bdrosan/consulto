<x-app-layout>
    <div class="p-4 mb-4 bg-white flex flex-wrap items-center gap-4">
        <a href="{{ route('follow-up.index') }}" class="flex items-center gap-x-2 text-sm uppercase hover:text-indigo-600 font-semibold tracking-widest outline-none focus:border-purple-200 focus:outline-none active:border-transparent active:text-grey-900 transition-all">
        ...
        <a href="{{ route('follow-up.createByLead',$id) }}" class="flex items-center gap-x-2 text-sm uppercase hover:text-indigo-600 font-semibold tracking-widest outline-none focus:border-purple-200 focus:outline-none active:border-transparent active:text-grey-900 transition-all">
        ...
        <a href="{{ route('appointment.leadShow',$id) }}" class="flex items-center gap-x-2 text-sm uppercase hover:text-indigo-600 font-semibold tracking-widest outline-none focus:border-purple-200 focus:outline-none active:border-transparent active:text-grey-900 transition-all">
        ...
        <div class="col-span-2">
			...
			C512,237.745,454.954,180.699,384.834,180.699z" />
                ...
                <form action="{{route('follow-up.transfer',$id)}}" method="post" class="md:flex">
                    @csrf
                    @method('PATCH')
                    <select name="user_id" class="border border-red-400 md:border-r-0 focus:border-red-400 focus:ring-red-400">
                        <option value="">Transfer Ownership</option>
                        @foreach($users as $user)
                        <option value="{{$user->id}}">{{$user->name}}</option>
                        @endforeach
                    </select>
                    <button type="submit" class="mt-2 md:mt-0 flex items-center gap-2 p-2 text-sm uppercase text-red-400 font-semibold tracking-widest outline-none border border-red-600 hover:text-white hover:bg-red-600 focus:border-red-200 focus:outline-none active:border-transparent active:text-grey-900 transition-all">
